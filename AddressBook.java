...
import address.view3.AddressCtrl;
...
public class AddressBook extends JFrame {
    ...
    private void initComponents() {
		...
		labelTimer = new JLabel("현재 시간 : "+date);
	...
	private void addActionPerformed(ActionEvent evt) {
		...
		mDialog.setVisible(true);
		ModifyDialog mo = new ModifyDialog();
		AddressVO vo = new AddressVO();
...
//				Integer id= (Integer)myTableModel.getValueAt(index[0], 0);
				...
				AddressCtrl ctrl = new AddressCtrl();
				ctrl.send(vo);
				mDialog.set("수정", true, ctrl.send(vo), abook); // 수정할 수 있게 손 봄
				mDialog.setVisible(true);
...
//				if(id == vos[0].getId()) {
//					newVo = new AddressVO(vos[0].getName(),vos[0].getAddress(),vos[0].getTelephone()
//							             ,vos[0].getGender(),vos[0].getRelationship(),vos[0].getRegistedate()
//							             ,vos[0].getBirthday(), vos[0].getComments(), vos[0].getId());
//				}
